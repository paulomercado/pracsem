...
def create_parser():
    ...
    p.add_argument('-w','--window', type=int, required=True)
    return p


# Sample execution: ./beta.py -f 20160104 -s 10038243 10029434 10033653 10009241 10036975 -o /q/home/ph.paulo.mercado/Aug30 -w 252
...
if __name__ == '__main__':
    ...
    d=utils.Dates()
    dates = d.date_range(d.trading_day_offset(dte,-w),d.trading_day_offset(dte,5))
    
    data = utils.read_data(PRICES_PATH,dates,columns=['date','cwiq_code','closing_price','splits','dividends'])
    macro = utils.read_data(MACROECONOMIC_PATH,dates,columns=['date','SPX'])
    ...
    for s in sids:
        ...
        rolling_var = df_final['SPX_log_ret'].rolling(window=w).var()
        df_final[f'beta_SPX_{w}'] = rolling_cov / rolling_var
    ...
    result.to_csv(f"{output_directory}/{dte.strftime('%Y%m%d')}.beta.csv",
                                      index = False,
                                      header = True
                                      )  
